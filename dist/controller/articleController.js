...
const summaryService = new summaryService_1.SummaryService();
class ArticleController {
    ...
    async getArticles(req, res) {
        try {
            const { limit = '10', offset = '0', page, search, tags, author } = req.query;
            const filters = {};
            if (search) {
                filters.$or = [
    ...
    async getArticleById(req, res) {
        try {
            ...
            const article = await Article_1.default.findById(id).select('-__v');
            if (!article) {
                res.status(404).json({
                    success: false,
    ...
    async updateArticle(req, res) {
        try {
            ...
            if (updates.author)
                cleanUpdates.author = updates.author.trim();
            if (updates.summary)
                cleanUpdates.summary = updates.summary.trim();
            if (updates.tags)
                cleanUpdates.tags = updates.tags.map(tag => tag.trim().toLowerCase());
            ...
            const article = await Article_1.default.findByIdAndUpdate(id, { ...cleanUpdates, updatedAt: new Date() }, { new: true, runValidators: true }).select('-__v');
            if (!article) {
                res.status(404).json({
                    success: false,
                    message: 'Article not found'
    ...
    async getArticlesByTags(req, res) {
        try {
            ...
            const articles = await Article_1.default.find({
                ...
                .skip(Number(offset))
                .select('-__v');
            const total = await Article_1.default.countDocuments({
                tags: { $in: tagArray }
            });
            res.json({
                success: true,
    ...
    async bulkRegenerateSummaries(req, res) {
        try {
            const { batchSize = 5, maxSentences = 3 } = req.body;
            const articlesNeedingSummaries = await Article_1.default.find({
                $or: [
    ...
    async getArticleStats(req, res) {
        try {
            const [totalArticles, totalAuthors, averageContentLength, tagStats, recentArticles, articlesWithSummaries] = await Promise.all([
                Article_1.default.countDocuments(),
                Article_1.default.distinct('author').then(authors => authors.length),
